...
The compression benchmark validates that compression preserves what the model needs. The suite holds 87 fixtures across 22 categories. Each fixture defines raw output, a must-preserve list, a must-not-contain list (which catches hallucination), and a minimum compression ratio. A fixture passes only when all three checks hold.
...
| Benchmark fixtures | 87 across 22 categories |
